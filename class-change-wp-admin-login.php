...
if ( defined( 'ABSPATH' ) && ! class_exists( 'Change_WP_Admin_Login' ) ) {
	...
	class Change_WP_Admin_Login {
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'change-wp-admin-login' ), '<strong>' . __( 'تغییر آدرس ورود', 'change-wp-admin-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'تغییر آدرس ورود وردپرس', 'تغییر آدرس ورود وردپرس' ) . '</h3>';
			$out .= '<p>' . __( 'تغییر آدرس صفحه ورود پیش فرض وردپرس', 'change-wp-admin-login' ) . '</p>';
		...
		public function admin_init() {
			...
			add_settings_section(
				...
				__( 'تغیر آدرس ورود', 'change-wp-admin-login' ),
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'آدرس ورود', 'change-wp-admin-login' ) . '</label>',
		...
		public function rwl_redirect_func() {
			...
			echo '<p class="description"><strong>' . __( 'اگر قسمت بالا را خالی بگذارید ، افزونه یک تغییر مسیر به صفحه اصلی وب سایت اضافه می کند.' ) . '</strong></p>';
		}
